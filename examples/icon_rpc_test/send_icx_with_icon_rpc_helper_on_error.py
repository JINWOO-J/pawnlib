...
# )
#
# payload = generator.json_rpc(**json_payload)
# signed_transaction = icon_rpc.sign_tx(payload=payload)
# pawn.console.log(f"icon_rpc.on_error => {icon_rpc.on_error}")
...
pawn.console.rule("Invalid network")
icon_rpc = IconRpcHelper(
    # network_info=NetworkInfo(network_name="cdnet", endpoint="INVALID_NETWORK_API_ENDPOINT"),
    network_info=NetworkInfo(network_name="cdnet"),
    wallet=icx_signer.load_wallet_key(private_key),
    raise_on_failure=False,
)

signed_transaction = icon_rpc.sign_tx(
    payload=generator.json_rpc(**json_payload)
)
